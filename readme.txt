...
Windows95, 98, Me, NT4.0, 2000, XP で動作します。

ダウンロードしたファイルを適当なフォルダに展開して、その中の CLCL.exe を実行す
るとタスクトレイにクリップのアイコンが表示されます。
初めて起動するとCLCLのフォルダ内にユーザ名のフォルダが作成され、ユーザ毎の設定
内容が保存されます。

レジストリは使用していないのでアンインストールを行うときは CLCL.exe が存在する
フォルダごと削除して構いません。
...
が表示されます。
このタスクトレイのアイコンを右クリックするとメニューが表示されます。
初期設定では、メニューに履歴が昇順に表示されます。
メニューは設定でカスタマイズすることができます。

タスクトレイのアイコンを左クリックするとビューアが表示されます。
...
・クリップボードとは？
	...
	例えば、メモ帳でコピーした文字列はWordなどに貼り付けができますが、これ
	はクリップボードという領域を利用しているからです。
...
・クリップボード形式
	...
	例えば、メモ帳でテキストをコピーした場合、
		...
		・LOCALE
		・OEM TEXT
	という３つの形式がクリップボードに入ります(Windows98の場合)
...
クリップボード形式はオプションの「フィルタ」で追加に設定してあるものだけを履歴
に追加します。
...
にします。
アイテムの名前を「-」にするとメニューで区切りとして表示します。アイテム内の形式
やデータは無視します。
名前に & をつけるとその次の文字をメニューでのショートカットキーとします。& その
ものをメニューに表示する場合は && としてください。
...
細な内容をツールチップで表示します。キーボードで項目を選択した場合は、メニュー
項目の下にツールチップを表示します。
...
「キャレット位置に表示」は、メニューをエディタのキャレットの位置に表示します。
設定しない場合はマウスの位置にメニューを表示します。
...
クリップボード形式は、ビューアでバイナリのダンプを表示します。
...
・履歴に追加しない
	設定したウィンドウでコピー操作しても履歴に追加しないようになります。
	履歴に入れることで不具合が生じるアプリケーションはこのオプションを指定
	しておくとそのアプリケーションからのコピーは無視するようになります。

・フォーカスを設定しない
	設定したウィンドウでアクティブにした後のフォーカスの設定をしないように
	なります。
...
・ツールをキャンセルしても貼り付けをする
	...
	けはしませんが、このオプションを指定するとキャンセルしても貼り付けをす
	るようになります。
...
コピーと貼り付けのキーは、設定されていないとデフォルトのキー設定が使われます。
...
既にCLCLが起動している場合は、既に起動しているCLCLにコマンドを送ります。
