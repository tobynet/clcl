...
BOOL ini_get_option(TCHAR *err_str)
{
	...
	if (option.format_cnt < 0) {
		...
		(option.format_info + 0)->format_name = alloc_copy(TEXT("TEXT"));
		(option.format_info + 0)->func_header = alloc_copy(TEXT("text_"));

		(option.format_info + 1)->format_name = alloc_copy(TEXT("BITMAP, DIB"));
		(option.format_info + 1)->func_header = alloc_copy(TEXT("bitmap_"));

		(option.format_info + 2)->format_name = alloc_copy(TEXT("DROP FILE LIST"));
		(option.format_info + 2)->func_header = alloc_copy(TEXT("file_"));
	...
	if (option.filter_cnt < 0) {
		...
		(option.filter_info + 0)->format_name = alloc_copy(TEXT("TEXT"));
	...
	option.bin_font_name = profile_alloc_string(TEXT("binview"), TEXT("font_name"), TEXT(""), ini_path);
