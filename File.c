...
static BOOL file_item_to_file(const HANDLE hFile, DATA_INFO *di, TCHAR *err_str)
{
	...
	for (; di != NULL; di = di->next) {
		...
		len++;		// \0
		_itot(di->plugin_param, str_param, 10);
		len += tchar_to_char_size(str_param);
		len++;		// \0
		tp = str_op;
		_itot(di->op_modifiers, tp, 10);
		tp += lstrlen(tp);
		*(tp++) = TEXT(',');
		_itot(di->op_virtkey, tp, 10);
		tp += lstrlen(tp);
		*(tp++) = TEXT(',');
		_itot(di->op_paste, tp, 10);
		...
		*(p++) = '\0';
		// 更新日時
		_itot(di->modified.dwHighDateTime, str_date, 16);
		for (i = 0; i < 8 - lstrlen(str_date); i++) {
			*(p++) = '0';
		}
		tchar_to_char(str_date, p, tchar_to_char_size(str_date));
		p += lstrlen(str_date);
		_itot(di->modified.dwLowDateTime, str_date, 16);
		for (i = 0; i < 8 - lstrlen(str_date); i++) {
			*(p++) = '0';
		}
		tchar_to_char(str_date, p, tchar_to_char_size(str_date));
		...
		*(p++) = '\x2';
		...
		for (cdi = di->child; cdi != NULL; cdi = cdi->next) {
			...
			len++;			// \0
			_itot(cdi->plugin_param, str_param, 10);
			len += tchar_to_char_size(str_param);
			len++;			// \0
			tp = str_op;
			_itot(cdi->op_modifiers, tp, 10);
			tp += lstrlen(tp);
			*(tp++) = TEXT(',');
			_itot(cdi->op_virtkey, tp, 10);
			tp += lstrlen(tp);
			*(tp++) = TEXT(',');
			_itot(cdi->op_paste, tp, 10);
			len += tchar_to_char_size(str_op);
			len++;			// \0
			len++;			// \x3
