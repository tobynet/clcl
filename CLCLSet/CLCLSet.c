...
static void set_general_option(const HINSTANCE hInstance);
...
static void get_work_path(const HINSTANCE hInstance)
{
	...
	*r = TEXT('\0');
	...
	// 作業ディレクトリの設定
	wsprintf(work_path, TEXT("%s\\%s"), tmp_path, user_name);
	if (file_check_directory(work_path) == FALSE) {
		CreateDirectory(work_path, NULL);
	}
}

/*
 * set_general_option - 共通設定の保存
 */
static void set_general_option(const HINSTANCE hInstance)
{
	TCHAR general_ini_path[MAX_PATH];
	...
	// ユーザ名の保存
	profile_get_string(TEXT("GENERAL"), TEXT("User"), TEXT(""), buf, BUF_SIZE - 1, general_ini_path);
	if (*buf == TEXT('\0') && general_option == 1) {
		profile_write_string(TEXT("GENERAL"), TEXT("User"), user_name, general_ini_path);
	} else if (*buf != TEXT('\0') && general_option == 0) {
		profile_write_string(TEXT("GENERAL"), TEXT("User"), TEXT(""), general_ini_path);
		*buf = TEXT('\0');
	}
	if (*buf != TEXT('\0')) {
		lstrcpy(user_name, buf);
...
#ifdef UNICODE
	p = alloc_char_to_tchar(lpCmdLine);
#else	// UNICODE
	p = lpCmdLine;
#endif	// UNICODE
	// オプション画面を表示
