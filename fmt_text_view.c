...
static int draw_string(const HWND hWnd, const HDC mdc, const BUFFER *bf, const int left, const int top, RECT *drect, TCHAR *str, const int len, const BOOL sel);
static void draw_line(const HWND hWnd, const HDC mdc, BUFFER *bf, const int i, const int w);
...
static int draw_string(const HWND hWnd, const HDC mdc, const BUFFER *bf, const int left, const int top, RECT *drect, TCHAR *str, const int len, const BOOL sel)
{
	...
	if (left + sz.cx >= 0) {
		...
		drect->left = left;
		drect->right = left + sz.cx;
		ExtTextOut(mdc, left, top, 0, drect, str, len, NULL);
		if (sel == TRUE) {
...
static void draw_line(const HWND hWnd, const HDC mdc, BUFFER *bf, const int i, const int w)
{
	...
	width = (bf->pos_x * bf->char_width) + w;
	height = (i - bf->pos_y) * bf->font_height + (bf->spacing / 2) + bf->top_margin;
	drect.top = (i - bf->pos_y) * bf->font_height + bf->top_margin;
	...
	for (j = line_get(bf, i), s = p = index_to_char(bf, line_get(bf, i)); j < line_get(bf, i + 1) && offset <= width; j++, p = char_next(bf, p)) {
		if (s != p) {
			if (bf->ip != NULL && (p == bf->ip + bf->ip_len || p == bf->input_buf)) {
				...
				offset += draw_string(hWnd, mdc, bf, offset, height, &drect, s, r - s, bf->sel);
				s = p;
			} else if (bf->dp != NULL && p == bf->dp + bf->del_len) {
				// 削除文字列
				offset += draw_string(hWnd, mdc, bf, offset, height, &drect, s, bf->dp - s, bf->sel);
				s = p;
			} else if ((j % DRAW_LEN) == 0) {
				offset += draw_string(hWnd, mdc, bf, offset, height, &drect, s, p - s, bf->sel);
				s = p;
			}
		}
		...
		if (*p == TEXT('\r') || *p == TEXT('\n')) {
			...
			if (bf->sel == TRUE && offset + bf->char_width >= 0) {
				draw_rect(hWnd, mdc, bf, offset, height - (bf->spacing / 2),
					offset + bf->char_width, height - (bf->spacing / 2) + bf->font_height);
			}
			break;
		} else if (*p == TEXT('\t')) {
			// Tab
			...
			if (bf->sel == TRUE && offset + tab >= 0) {
				draw_rect(hWnd, mdc, bf, offset, height - (bf->spacing / 2),
					offset + tab, height - (bf->spacing / 2) + bf->font_height);
			}
			offset += tab;
	...
	draw_string(hWnd, mdc, bf, offset, height, &drect, s, r - s, bf->sel);
}
...
static void caret_move(const HWND hWnd, BUFFER *bf, const int key)
{
	...
	case VK_PRIOR:
		...
		bf->cp = caret_point_to_caret(bf, pt.x, pt.y - (rect.bottom - rect.top));
	...
	case WM_PAINT:
		if ((bf = (BUFFER *)GetWindowLong(hWnd, GWL_USERDATA)) != NULL) {
			...
			for (; i < bf->line_len && i < bf->pos_y + (ps.rcPaint.bottom / bf->font_height) + 1; i++) {
				...
				// 1行描画
				draw_line(hWnd, bf->mdc, bf, i, rect.right);
			}
			FillRgn(bf->mdc, bf->hrgn, hBrush);
			DeleteObject(hBrush);
